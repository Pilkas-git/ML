...
def evaluate(model, data_loader, device):
    ...
    for i, (image, targets) in enumerate(data_loader):
        ...
        for target in outputs:
            det_boxes.append(target["boxes"])
            det_labels.append(target["labels"])
            det_scores.append(target["scores"])
    ...
    APs, mAP = calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, device)
    torch.set_num_threads(n_threads)
    print("Averaged stats:", APs)

    return APs, mAP 

def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, device):
    """
    ...
    for c in range(1, n_classes):
        ...
        n_easy_class_objects = len(true_class_images) 
